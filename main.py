...
DB = {
    ...
    "user": "redvon1216",
    "password": os.environ.get("DB_PASS", "여기에실패시쓸패스워드"),  # Web탭 → Environment Variables에 DB_PASS 등록 권장
...
def api_create_word():
    ...
    reg = (data.get("registered_on") or "").strip()[:10]  # 프론트는 registered_on을 보냄
    ...
    with get_conn() as conn, conn.cursor() as cur:
        # created_at 컬럼에 저장
        cur.execute(
            "INSERT INTO words (word, meaning, example, level, created_at) VALUES (%s,%s,%s,%s,%s)",
...
def api_stats_daily():
    ...
    for r in rows:
        tot = (r.get("correct") or 0) + (r.get("wrong") or 0)
        r["accuracy"] = round((r.get("correct") or 0) * 100 / tot, 1) if tot else 0
    return jsonify(rows)
